/**
 * Utilitários criptográficos seguros para senhas e autenticação do MES Bella Top
 * Utiliza SHA-256 com salt único por usuário para armazenamento irreversível.
 */
...
// Implementação determinística de SHA-256 (compatível com Browser e Node)
export async function sha256Hex(message: string): Promise<string> {
  ...
  // Algoritmo puro de SHA-256 em JS para máxima confiabilidade síncrona/fallback
  return sha256Sync(message);
}

// SHA-256 síncrono puro
...
/**
 * Cria o hash da senha combinando senha pura + salt
 */
export function hashPassword(plainText: string, salt: string): string {
...
export function verifyPassword(plainText: string, storedHash: string, salt: string): boolean {
  ...
  const computed = hashPassword(plainText, salt);
  return computed === storedHash;
}

/**
 * Normaliza o nome de login para evitar inconsistências de maiúsculas/minúsculas, acentos, espaços e caracteres especiais
 */
